...
make install
chroot_check $? "system-build : glibc : make install"
...
cp -v /opt/admin/physix/build-groups/02-base-system/2.040-glibc/etc_nsswitch.conf /etc/nsswitch.conf
...
cp -v /opt/admin/physix/build-groups/02-base-system/2.040-glibc/etc_ld.so.conf  /etc/ld.so.conf
