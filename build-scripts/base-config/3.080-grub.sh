...
if [ $CHOICE == 'yes' ] ; then
	...
	cp -v /physix/build-scripts/base-config/configs/lvm-grub.cfg /boot/grub/grub.cfg
...
if [ -e /boot/grub ] ; then
        cp -v /physix/build-scripts/base-config/configs/unicode.pf2 /boot/grub/fonts
	chroot_check $? "cp -v /physix/build-scripts.config/configs/unicode.pf2 /boot/grub/fonts"

        cp -v /physix/build-scripts/base-config/configs/physix-splash.png /boot/grub/
