...
#CREATE LOGICAL VOLUES
	...
	#CREATE /opt LOGICAL VOLUME
        if [ $CONF_LOGICAL_OPT_SIZE ] ; then
                lvcreate -L "$CONF_LOGICAL_OPT_SIZE"G -n opt $CONF_VOL_GROUP_NAME
                if [ $? -ne 0 ] ; then
                        report "lvcreate -L "$CONF_LOGICAL_OPT_SIZE"G -n opt $CONF_VOL_GROUP_NAME"
                        exit 1
                fi
        else
                lvcreate -L 20G -n opt $CONF_VOL_GROUP_NAME
                if [ $? -ne 0 ] ; then
                        report "lvcreate -L 4G -n usr $CONF_VOL_GROUP_NAME"
                        exit 1
                fi
...
#FORMAT VOLUMES
	...
	if [ $? -ne 0 ] ; then
        ...
        mkfs.$CONF_ROOTPART_FS /dev/mapper/"$CONF_VOL_GROUP_NAME"-opt
        if [ $? -ne 0 ] ; then
                report "[ERROR] $MKFS /dev/mapper/physix-opt"
                exit 1
        fi
...
#MOUNT VOLUMES
	...
	if [ $? -ne 0 ] ; then
        ...
        mount /dev/mapper/"$CONF_VOL_GROUP_NAME"-usr $BUILDROOT/opt
        if [ $? -ne 0 ] ; then
                report "[ERROR] mount /dev/mapper/physix-usr $BUILDROOT/opt"
                exit 1
        fi
...
function complete_setup() {
	...
	mkdir -vp $BUILDROOT/opt/system-build-logs
	mkdir -vp $BUILDROOT/opt/sources
...
------
	...
	cp -v $BUILDROOT/physix/build-scripts/03-base-config/configs/physix-bash-profile  /home/physix/.bash_profile
	if [ $? -ne 0 ] ; then
		echo "Error Creating /home/physix/.bash_profile"
		exit 1
	fi
	cp -v $BUILDROOT/physix/build-scripts/03-base-config/configs/physix-bashrc  /home/physix/.bashrc
	if [ $? -ne 0 ] ; then
