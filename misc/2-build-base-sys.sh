...
NUM_SCRIPTS=`ls $BUILDROOT/physix/build-scripts/02-base/ | wc -l`
BUILD_ID=0
TIME=`date "+%D-%T"`
report "$TIME : $BUILD_ID : Building 2.000-base-build-prep.sh"

# Called differently becuase it is not chrooted
if [ $START_POINT -eq 0 ] ; then
	report "Running: 2.000-base-build-prep.sh"
	./build-scripts/02-base/2.000-base-build-prep.sh
	check $? "2.000-base-build-prep.sh"
fi
