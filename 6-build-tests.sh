...
START_POINT=${1:-0}
STOP_POINT=`wc -l ./6-build-tests.csv | cut -d' ' -f1`
NUM_SCRIPTS=`ls /physix/build-scripts.tests/ | wc -l`
BUILD_ID=0
for LINE in `cat ./6-build-tests.csv | grep -v -e '^#' | grep -v -e '^\s*$'` ; do
	...
	if [ $BUILD_ID -ge $START_POINT ] && [ $BUILD_ID -le $STOP_POINT ] ; then
		...
		if [ ! -e /physix/build-scripts.tests/$SCRIPT ] ; then
			report "Build Script NOT found: /build-scripts.tests/$SCRIPT"
			exit 1
		fi

		eval "/physix/build-scripts.tests/$SCRIPT $SRC0 $PKG1 $PKG2 $IO_DIRECTION"
...
report "-------------------------------------"
report "- System Tests Build complete  -"
report "-------------------------------------"
